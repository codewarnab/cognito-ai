...
import { detectPdfInTab, pdfDebouncer } from './utils/pdfDetector';

/**
 * Listen for tab updates and detect PDF URLs
 * When a PDF is detected, fetch it and convert to base64 (like file attachments)
 */
chrome.tabs.onUpdated.addListener(async (tabId, changeInfo, _tab) => {
    ...
    try {
        // Detect if tab contains PDF
        const pdfInfo = await detectPdfInTab(tabId);

        if (pdfInfo) {
            console.log('[Background] ✅ PDF detected:', pdfInfo.url);

            // Process PDF with debouncing (like file attachments)
            await pdfDebouncer.scheduleProcessing(pdfInfo.url, async () => {
                try {
                    console.log('[Background] Starting PDF fetch:', pdfInfo.url);

                    // Notify sidepanel processing started
                    chrome.runtime.sendMessage({
                        type: 'PDF_PROCESSING',
                        data: { url: pdfInfo.url }
                    }).catch(() => {
                        console.debug('[Background] No listeners for PDF_PROCESSING');
                    });
                    ...
                    // Check size (20MB limit for inline data)
                    const maxSize = 20 * 1024 * 1024;
                    if (sizeBytes > maxSize) {
                        throw new Error(`PDF too large: ${(sizeBytes / 1024 / 1024).toFixed(1)}MB (max 20MB for inline)`);
                    }

                    // Convert to base64 (same as file attachments)
                    ...
                    console.log('[Background] PDF fetched successfully:', {
                        url: pdfInfo.url,
                    ...
                    await chrome.storage.local.set({
                        current_pdf_context: pdfContext
                    });

                    // Notify sidepanel of successful processing
                    chrome.runtime.sendMessage({
                        type: 'PDF_PROCESSED',
                        data: pdfContext
                    }).catch(() => {
                        console.debug('[Background] No listeners for PDF_PROCESSED');
                    });
                } catch (error) {
                    ...
                    chrome.runtime.sendMessage({
                        ...
                        console.debug('[Background] No listeners for PDF_ERROR');
                    });
                }
            });
        }
    } catch (error) {
        console.error('[Background] Error in PDF detection:', error);
    }
});

console.log('[Background] PDF detection listener initialized');
