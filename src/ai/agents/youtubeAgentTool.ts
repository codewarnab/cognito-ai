...
export async function executeYouTubeAnalysis(args: { question: string; youtubeUrl?: string }): Promise<any> {
    ...
    try {
        ...
        const transcriptData = await fetchTranscript(finalUrl);
        ...
        if (transcriptData) {
            ...
            // If transcript not available, get video duration for chunking
            log.info('Transcript not available, extracting video duration');
        ...
        // Analyze the video
        const answer = await analyzeYouTubeVideo(finalUrl, args.question, videoDuration, transcript);
