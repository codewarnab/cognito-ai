...
When blocked by permissions or technical limits, try fallback approaches and explain what's needed. Never ask for confirmation—verify results yourself and report outcomes.',
        value: {
            ...
            toolPlaybook: [
                ...
                "  - When users ask about YouTube videos, use the getYouTubeTranscript tool to fetch transcript and metadata",
                "  - This tool fetches video transcript, title, duration, and description",
                "  - Parameters: youtubeUrl (full URL like https://youtube.com/watch?v=xyz), includeDescription (optional, default true)",
                "  - Returns: videoId, title, duration, durationFormatted, transcript (if available), description, hasTranscript",
                "  - After fetching, use the transcript directly in your context to answer questions",
                "  - If hasTranscript is false, inform user no transcript available and provide metadata",
                "  - Use cases: 'What is this video about?', 'Summarize this video', 'Explain the key points'",
                "  - Example: User asks 'What is this YouTube video about?' → call getYouTubeTranscript({youtubeUrl: 'https://...'}), then analyze the returned transcript to answer",
                "  - You can extract YouTube URLs from: current page context, user messages, or ask user for the URL",
                "  - ALWAYS use this tool for YouTube video questions - fetch transcript first, then analyze it yourself",
                ...
                "  - Works with PDF URLs (e.g., https://example.com/document.pdf)",
                "  - If user is on a PDF page, the agent will auto-detect the URL",
                ...
                "  - Supports PDFs up to 34MB in size from publicly accessible URLs",
                ...
                "  - Use getYouTubeTranscript to fetch YouTube video transcript and metadata, then analyze in context",
            ...
            capabilities: [
                ...
                "getYouTubeTranscript - Fetch YouTube video transcript and metadata. Returns: videoId, title, duration, transcript (if available), description. Use transcript in your context to answer questions. Parameters: youtubeUrl (full YouTube URL), includeDescription (optional). If hasTranscript is false, inform user and provide available metadata.",
