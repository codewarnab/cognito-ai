...
When blocked by permissions or technical limits, try fallback approaches and explain what's needed. Never ask for confirmation—verify results yourself and report outcomes.',
        value: {
            ...
            toolPlaybook: [
                ...
                "    * IF PDF is already attached inline (user message has PDF attachment) → YOU CAN ANSWER! Use the attached content directly.",
                "    * IF user is on local file page (file://) AND no PDF attached → ask them to attach via icon",
                "    * IF user provides local file path in text → ask them to attach via icon",
                "  - NEVER deny responding about PDFs when a PDF is already attached inline in the conversation!",
                ...
                "  - LOCAL FILE PATH HANDLING (only when PDF NOT already attached):",
                "    * file:///C:/Users/User/Downloads/document.pdf (no attachment) → ASK FOR ATTACHMENT",
                "    * file://localhost/Users/... (no attachment) → ASK FOR ATTACHMENT",
                "    * blob:https://... (no attachment) → ASK FOR ATTACHMENT",
                "    * BUT if PDF is already attached inline → ANSWER THE QUESTION using attached content!",
                "  - REMEMBER: Check for inline PDF attachments BEFORE rejecting local file requests!",
