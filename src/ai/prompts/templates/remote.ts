...
function buildErrorRecovery(): string {
  const strategies = [
    ...
    "ELEMENT NOT FOUND: Try alternative interaction methods. If all fail, extract page content and report.",
...
function buildCapabilities(): string {
  const capabilities = [
    ...
    "",
    "MEMORY:",
    "  • Save information to persistent memory (requires user consent)",
    "  • Retrieve saved memories",
    "  • List and filter memories by category",
    "  • Delete memories",
    "  • Suggest saving useful information",
    "",
    "REMINDERS:",
