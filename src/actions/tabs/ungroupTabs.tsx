    ...
    useEffect(() => {
        ...
        registerToolUI('ungroupTabs', (state: ToolUIState) => {
            ...
            if (toolState === 'input-streaming' || toolState === 'input-available') {
                const subtitle = input?.ungroupAll ? "Ungrouping all groups..." :
                    input?.groupIds ? `Ungrouping ${Array.isArray(input.groupIds) ? input.groupIds.length : 1} group(s)...` :
                        "Ungrouping...";
                return (
                    <ToolCard 
                        title="Ungrouping Tabs" 
                        subtitle={subtitle} 
                        state="loading" 
                        icon="🔓" 
                    />
            ...
            if (toolState === 'output-available' && output) {
                ...
                if (output.success) {
                    if (output.ungroupedCount === 0) {
                        return (
                            <ToolCard 
                                title="No Groups to Ungroup" 
                    ...
                    return (
                        <ToolCard
                            ...
                            subtitle={`Ungrouped ${output.ungroupedCount} group(s), ${output.totalTabsUngrouped} tab(s) freed`}
                            state="success"
                            icon="🔓"
                        >
