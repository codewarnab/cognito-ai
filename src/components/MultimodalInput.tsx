...
// Button variants using cva - Updated for dark theme
const buttonVariants = cva(
  'inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0',
  {
    variants: {
      variant: {
        // Primary: Green Pulse background, black text
        default: 'bg-[#C6FE1E] text-black hover:bg-[#B8E61A]',
        // Destructive: red background, white text
        destructive:
          'bg-red-500 text-white hover:bg-red-600',
        // Outline: white border, transparent background, white text
        outline:
          'border border-white/20 bg-transparent text-white hover:bg-white/10',
        // Secondary: gray background, white text
        secondary:
          'bg-white/10 text-white hover:bg-white/20',
        // Ghost: hover effect, white text
...
// Textarea component - Updated for dark theme
...
>(({ className, ...props }, ref) => {
  return (
    <textarea
      className={cn(
        'flex min-h-[80px] w-full rounded-md border border-white/20 bg-transparent px-3 py-2 text-base ring-offset-background placeholder:text-white/50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#C6FE1E] focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 md:text-sm text-white',
        className,
      )}
      ref={ref}
      {...props}
      ...
      {suggestedActions.map((suggestedAction, index) => (
        ...
        >
          <Button
            ...
            className="text-left border border-white/20 rounded-xl px-4 py-3.5 text-sm flex-1 gap-1 sm:flex-col w-full h-auto justify-start items-start
...
const PreviewAttachment = ({
  ...
  return (
    <div data-testid="input-attachment-preview" className="flex flex-col gap-1">
      <div className="w-20 h-16 aspect-video bg-white/10 rounded-md relative flex flex-col items-center justify-center overflow-hidden border border-white/20">
        {contentType?.startsWith('image/') && url ? (
          ...
          <div className="flex items-center justify-center text-xs text-white/60 text-center p-1">
        ...
        {isUploading && (
          <div
            ...
            className="animate-spin absolute text-white/60"
          >
            <LoaderIcon className="size-5" />
          </div>
        )}
      </div>
      <div className="text-xs text-white/60 max-w-20 truncate">
        {name}
      </div>
    </div>
  );
...
function PureAttachmentsButton({
  ...
  return (
    <Button
      ...
      className="rounded-md rounded-bl-lg p-[7px] h-fit border border-white/20 hover:bg-white/10"
      onClick={(event) => {
